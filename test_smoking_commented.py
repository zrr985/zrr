...
class SmokingDetectionTest:
    """吸烟检测测试类"""
    
    def __init__(self):
        """初始化测试类的各种参数和队列"""
        self.class_smoke = 0                        # 吸烟检测结果，0=无吸烟，1=检测到吸烟，None=不确定
    ...
    def smoking_detection_task(self):
        """吸烟检测任务线程函数"""
        print("吸烟检测任务启动")                   # 打印任务启动信息
        model_path = "./smoking.rknn"             # RKNN吸烟检测模型文件路径
        ...
        while not self.stop_event.is_set():       # 当停止事件未设置时，持续运行
            try:
                ...
                processed_frame, self.class_smoke = result  # 解包结果：处理后的图像和检测结果
                
                if not flag:                       # 如果处理失败
                    break                          # 跳出循环
                
                # 检测结果分析和状态文本生成
                if self.class_smoke == 1:          # 如果检测到吸烟行为
                    abnormal_count += 1            # 异常帧计数器加1
                    status_text = f"Smoking Detected! (Count: {abnormal_count})"  # 生成英文状态文本
                    color = (0, 0, 255)            # 设置文本颜色为红色（BGR格式）
                    
                    if abnormal_count >= 20:       # 如果连续异常帧数达到阈值（吸烟检测阈值设为20帧）
                        print(f"警告: 连续检测到吸烟行为{abnormal_count}帧!")  # 打印警告信息
                elif self.class_smoke == 0:       # 如果明确无吸烟行为
                    abnormal_count = 0             # 重置异常帧计数器
                    status_text = "Smoking: Normal"  # 生成英文正常状态文本
                    color = (0, 255, 0)            # 设置文本颜色为绿色
                else:                              # 如果检测结果不确定（None）
                    status_text = "Smoking: Unknown"  # 生成英文不确定状态文本
                ...
                cv2.putText(processed_frame, f"Detection Value: {self.class_smoke}", (10, 110),  # 显示检测值
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)  # 白色文本
                
                # 将处理好的帧和任务代号一起放入显示队列（与main.py一致）
                self.display_queue.put((processed_frame, 5))  # 吸烟检测任务代号为5
                
                frames += 1                        # 帧计数器加1
                if frames % 30 == 0:               # 每30帧计算一次帧率
                    fps = 30 / (time.time() - loopTime)  # 计算帧率
                    print(f"30帧平均帧率: {fps:.2f} 帧/秒, 当前检测值: {self.class_smoke}")  # 打印帧率和检测值
                    loopTime = time.time()         # 重置计时起点
                    
            except queue.Empty:                    # 如果队列为空（超时）
                continue                           # 继续下一次循环
        
        print("吸烟检测任务结束")                   # 打印任务结束信息
    ...
    def run_test(self):
        """运行测试的主函数"""
        print("=== 吸烟检测测试程序 ===")            # 打印程序标题
        print("按 'q' 键退出程序")                  # 打印操作提示
        print("检测值说明: 1=检测到吸烟, 0=无吸烟, None=不确定")  # 打印检测值说明
        print("程序会检测香烟、人脸和吸烟动作的组合来判断吸烟行为")  # 打印功能说明
        ...
        try:
            ...
            # 启动检测任务线程
            detection_thread = threading.Thread(target=self.smoking_detection_task)     # 创建检测线程
            ...
            cam.release()                          # 释放摄像头资源
            cv2.destroyAllWindows()                # 关闭所有OpenCV窗口
            print("吸烟检测测试程序结束")           # 打印程序结束信息

def main():
    """主函数"""
    test = SmokingDetectionTest()                  # 创建测试类实例
    test.run_test()                                # 运行测试
