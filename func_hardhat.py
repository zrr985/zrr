...
class HardhatDetector:
    ...
    def detect(self, rknn_lite, img, frame_num):
        ...
        self.detection_history.append(has_no_hardhat)
        ...
        # 计数
        #status_text = f"Hardhat: {class_real}"
        #cv2.putText(img, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            
        return img, class_real


hardhat_detector = HardhatDetector(detection_threshold=0.7, window_size=10)

def myFunc_hardhat(rknn_lite, img, frame_num):
    return hardhat_detector.detect(rknn_lite, img, frame_num)