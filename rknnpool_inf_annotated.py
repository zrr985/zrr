# -*- coding: utf-8 -*-
"""
RKNN模型池 - 红外入侵检测专用 (rknnpool_inf.py)
功能：管理YOLOv7模型池，实现红外摄像头的人体入侵检测并发处理
作者：视觉检测系统
日期：2024
"""
...
def initRKNN(rknnModel="./rknnModel/yolov5s.rknn", id=0):
    ...
    参数：
        rknnModel: RKNN模型文件路径（默认YOLOv7模型）
...
class rknnPoolExecutor_inf():
    ...
    def __init__(self, rknnModel, TPEs, func):
        ...
        # 初始化YOLOv7模型池
        self.rknnPool = initRKNNs(rknnModel, TPEs)  # YOLOv7模型池
    ...
    def put(self, frame):
        ...
        self.queue.put(self.pool.submit(
            self.func,  # 处理函数
            self.rknnPool[self.num % self.TPEs],  # YOLOv7模型实例
            frame,  # 输入图像
            self.num  # 任务编号
        ))
        self.num += 1  # 任务计数器递增
