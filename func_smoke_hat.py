...
def myFunc_smoke_hat(rknn_lite, IMG, num):
    ...
    if classes is not None:
        for cl in classes:
            if cl in [0, 1]:  # no_hardhat, hardhat
                hardhat_classes.append(cl)
            elif cl in [2, 3]:  # not_smoking, smoking
                smoke_classes.append(cl)
    
    # 根据调用上下文返回相应的结果
    # 这里我们返回所有检测结果，让调用方决定如何处理
    all_classes = []
    if classes is not None:
        for cl in classes:
