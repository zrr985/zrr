# -*- coding: utf-8 -*-
"""
RKNN模型池 - RGB摄像头专用 (rknnpool_rgb.py)
功能：管理RetinaFace和MobileFaceNet双模型池，实现人脸检测和识别的并发处理
作者：视觉检测系统
日期：2024
"""
...
class rknnPoolExecutor_face():
    """
    人脸识别专用的RKNN模型池执行器
    管理RetinaFace和MobileFaceNet双模型，实现人脸检测和特征提取的并发处理
    """
    
    def __init__(self, rknnModel1, rknnModel2, TPEs, func):
        """
        初始化人脸识别模型池
        
        参数：
            rknnModel1: RetinaFace模型文件路径（人脸检测）
            rknnModel2: MobileFaceNet模型文件路径（特征提取）
            TPEs: 线程池执行器数量
            func: 处理函数（人脸检测和识别函数）
        """
        ...
        # 初始化两个模型池：RetinaFace池和MobileFaceNet池
        self.rknnPool1 = initRKNNs(rknnModel1, TPEs)  # RetinaFace模型池
        self.rknnPool2 = initRKNNs(rknnModel2, TPEs)  # MobileFaceNet模型池
    ...
    def put(self, frame):
        ...
        self.queue.put(self.pool.submit(
            self.func,  # 处理函数
            self.rknnPool1[self.num % self.TPEs],  # RetinaFace模型实例
            self.rknnPool2[self.num % self.TPEs],  # MobileFaceNet模型实例
            frame,  # 输入图像
            self.num  # 任务编号
        ))
        self.num += 1  # 任务计数器递增
    ...
    def release(self):
        ...
        # 释放所有模型实例的资源
        for rknn_lite1, rknn_lite2 in zip(self.rknnPool1, self.rknnPool2):
            rknn_lite1.release()  # 释放RetinaFace模型
            rknn_lite2.release()  # 释放MobileFaceNet模型
